...
void SE_SearchValueProcess(U8 ucSEID)
{
    ...
    **********************************************************************/
    ...
    if ( TRUE == aFind[SE_2ND_MSK_GROUP] )
    {
        ...
        Comm_WriteReg( (U32)&rSE_SINDEX(ucSEID), 1, &(aRtnIndex[SE_2ND_MSK_GROUP]) );
        Comm_WriteReg( (U32)&rSE_RETURN_SVALUE_LOW(ucSEID), 1, &(aKeyValue[SE_2ND_MSK_GROUP].LowDw) );
        Comm_WriteReg( (U32)&rSE_RETURN_SVALUE_HIGH(ucSEID), 1, &(aKeyValue[SE_2ND_MSK_GROUP].HighDw) );
    }
    else
    {
