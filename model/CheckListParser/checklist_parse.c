...
extern void Mid_FlashSetBlockErase(U16 uMaxEraseCnt, U16 uMinEraseCnt, U32 ulMaxPercent);
//Mid_FlashSetErrRate
extern void Mid_FlashSetErrRate(U16 uUECCRate, U16 uRECCRate, U16 uPragramErrRate, U16 uEraseErrRate);
...
ERR_INJ_PARRM CRCErrParamSlot[ERR_INJ_SLOT_MAX];
U32 g_InjErrHead;
U32 g_InjErrTail;
U32 g_InjCRCErrHead;
U32 g_InjCRCErrTail;
FLASH_ERR_PARAM g_InjErrParamType;

HSRW_CMD_TYPE g_HSRWCmdType;

FCMD_COND* g_pFCOND;
char l_strCheckListFolder[255] = {0};
...
U32 GetParameters(char* pSrcStr, const char* pSeparator, char **ppDestStr, U8 usParamCnt)
{
    char *pTocken = NULL;
...
BOOL set_bp_param(char* BP_Desc,U16* pBPNum,FUNCTION_PARAMETER* pBpParam)
{
    ...
    if (TRUE == bFind)
    {
        ...
        ulParaCnt = GetParameters(buffer, &cTocken, pParamArray, 4);
...
void set_errrate_parm(char *pErrRateDesc, FLASH_ERR_RATE *pErrRateParam)
{
    char uTempBuffer[255] = { 0 };
    char UECCRate[255];
    char RECCRate[255];
    char ProgramErrRate[255];
    char EraseErrRate[255];
    char *pParamArray[4] = { UECCRate, RECCRate, ProgramErrRate, EraseErrRate};
    char cTocken = ',';
    U32 ulParaCnt;
    ...
    pdest = strchr(pErrRateDesc, '(');
    if (pdest == NULL)
    {

        pErrRateParam->EraseErrRate = 0;
        pErrRateParam->ProgramErrRate = 0;
        pErrRateParam->UECCRate = 0;
        pErrRateParam->RECCRate = 0;
        return;
    }

    index = (int)(pdest - pErrRateDesc);

    memcpy(uTempBuffer, pErrRateDesc + index + 1, strlen(pErrRateDesc) - index);

    ulParaCnt = GetParameters(uTempBuffer, &cTocken, pParamArray, 4);

    if (strncmp(UECCRate, "?", 1))
    {
        pErrRateParam->UECCRate = atoi(UECCRate);
    }
    else
    {
        pErrRateParam->UECCRate = 0;
    ...
    if (strncmp(RECCRate, "?", 1))
    {
        pErrRateParam->RECCRate = atoi(RECCRate);
    }
    else
    {
        pErrRateParam->RECCRate = 0;
    ...
    if (strncmp(ProgramErrRate, "?", 1))
    {
        pErrRateParam->ProgramErrRate = atoi(ProgramErrRate);
    }
    else
    {
        pErrRateParam->ProgramErrRate = 0;
    ...
    if (strncmp(EraseErrRate, "?", 1))
    {
        pErrRateParam->EraseErrRate = atoi(EraseErrRate);
    }
    else
    {
        pErrRateParam->EraseErrRate = 0;
...
void set_erasecnt_param(char *pEraseCntDesc, ERASECNT_SETTING *pEraseCntSetting)
{
    char buffer[255] = { 0 };
    char uMaxEraseCnt[255];
    char uMinEraseCnt[255];
    char uMaxPercent[255];
    char *pParamArray[3] = { uMaxEraseCnt, uMinEraseCnt, uMaxPercent};
    char cTocken = ',';
    U32 ulParaCnt;
    ...
    pdest = strchr(pEraseCntDesc, '(');
    if (pdest == NULL)
    {
        pEraseCntSetting->ulMaxPercent = 0;
        pEraseCntSetting->uMaxEraseCnt = 0;
        pEraseCntSetting->uMinEraseCnt = 0;
        return;
    }

    index = (int)(pdest - pEraseCntDesc);

    memcpy(buffer, pEraseCntDesc + index + 1, strlen(pEraseCntDesc) - index);

    ulParaCnt = GetParameters(buffer, &cTocken, pParamArray, 3);

    if (strncmp(uMaxEraseCnt, "?", 1))
    {
        pEraseCntSetting->uMaxEraseCnt = atoi(uMaxEraseCnt);
    }
    else
    {
        pEraseCntSetting->uMaxEraseCnt = 0;
    ...
    if (strncmp(uMinEraseCnt, "?", 1))
    {
        pEraseCntSetting->uMinEraseCnt = atoi(uMinEraseCnt);
    }
    else
    {
        pEraseCntSetting->uMinEraseCnt = 0;
    ...
    if (strncmp(uMaxPercent, "?", 1))
    {
        pEraseCntSetting->ulMaxPercent = atoi(uMaxPercent);
    }
    else
    {
        pEraseCntSetting->ulMaxPercent = 0;
...
BOOL get_HSRWCmdType(char *pCmdTypeName, U8 * pCmdType)
{
    HSRW_CMD_TYPE_DESC *pHSRWCmdTypeDesc;
    BOOL bFind = FALSE;
    pHSRWCmdTypeDesc = &HSRW_CMD_TYPE_LIB[0];
    while (strcmp(pHSRWCmdTypeDesc->CmdPrtDesc, "END"))
    {
        if (!strncmp(pHSRWCmdTypeDesc->CmdPrtDesc, pCmdTypeName, strlen(pHSRWCmdTypeDesc->CmdPrtDesc)))
        {
            *pCmdType = (U8)pHSRWCmdTypeDesc->CmdPrtType;
            bFind = TRUE;
            break;
        }
        pHSRWCmdTypeDesc++;
...
void parse_action(COND_LIST* tmp_condlist,char* action,char* param)
{
    ...
    while(*s)
    {
        ...
        if(*s == '/' || *s == '}')
        {
            ...
            else if(NULL != param && pACT->ActType == ACT_TYPE_INJECT_CRCERR)
            {
                if(FALSE == parse_injectCRCerror_param(pACT,param))
                {
                    printf("parse_injectCRCerror_param error!\n");
                    DBG_Break();
                }
            }
            ...
            else if (pACT->ActType == ACT_TYPE_CMD_TYPE)
            {
                strcpy(pACT->ProtocolType, Temp);
            }
            else if (pACT->ActType == ACT_TYPE_SET_ERASECNT)
            {
                // call erase cnt setting call back
                set_erasecnt_param(real_action, &pACT->m_FlashEraseCntParam);
            }
            else if (pACT->ActType == ACT_TYPE_SET_FLASH_ERR_RATE)
            {
                set_errrate_parm(real_action, &pACT->m_FlashErrRate);
            }
...
void add_hscmd_define(char* scmd_name,char* key,char* value)
{
    ...
    else if(!strcmp(key,"Data"))
    {
        ...
        if(strlen(value) > 2)
        {
            ...
            {
                sprintf(file_name,"%s/special_cmd_list/%s", l_strCheckListFolder, value);
                pFile = fopen(file_name,"r+b");
                if (pFile == NULL)
                {
                    DBG_Printf("file %s open fail.\n", file_name);
                    DBG_Getch();
                }
    ...
    while(sec != NULL)
    {
        ...
        while(condlist != NULL)
        {
            ...
            while(action != NULL)
            {
                ...
                else if(action->ActType == ACT_TYPE_INJECT_CRCERR)
                {
                    printf("LBA 0x%x CmdCode 0x%x ErrType %d\n", action->m_ErrInjParam.LBA, action->m_ErrInjParam.CmdCode, action->m_ErrInjParam.ErrType);
                }
...
{
    char FilePath[256] = {0};
    char strCheckListFolder[] = "checklist";
    ...
    //_getcwd(strCurPath, sizeof(strCurPath));
    GetModuleFileName(NULL, strCurPath, sizeof(strCurPath));

    // find checklist folder in the current path.
    do
    {
        pTempPose = strrchr(strCurPath, '\\');

        if (NULL == pTempPose)
        {
            DBG_Printf("Don't find checklist folder in the current path \n");
            DBG_Getch();
        }

        memset(pTempPose, 0, strlen(pTempPose));
        sprintf(strDstPath, "%s\\%s", strCurPath, strCheckListFolder);

    }while (0 != _access(strDstPath, 0));

    memcpy(l_strCheckListFolder, strDstPath, strlen(strDstPath));
    sprintf(FilePath, "%s\\*.*", l_strCheckListFolder);

    GetChecklistInit(FilePath);

    return;
}
void parse_scmd_list_ini()
{
    char strCmdFilePath[256] = {0};
#ifndef HOST_NVME
    sprintf(strCmdFilePath, "%s\\%s", l_strCheckListFolder, "special_cmd_list\\special_cmd_list.ini");
#else
    sprintf(strCmdFilePath, "%s\\%s", l_strCheckListFolder, "special_cmd_list\\special_cmd_list_nvme.ini");
#endif
    ...
    scmd_parser_load(strCmdFilePath);
}
...
void dump_checklist_info(char* fileName)
{
    ...
    sprintf(strFileName, "%s%s", pstrLogFolder, "\\RunChecklistInfo.txt");

    if(bInit == 0)
    {
        bInit = 1;
        pDump = fopen(strFileName, "w");
    }
    else
    {
        pDump = fopen(strFileName, "a+");
    }
...
BOOL load_one_checklist()
{
    ...
    if ((file_name[0] != '@') && (file_name[0] != '!'))
    {
        ...
        sprintf(file_dstname, "%s\\%s", l_strCheckListFolder, file_name);
...
BOOL do_one_action()
{
    ...
    case ACT_TYPE_SETBP:
        ...
        {
            DBG_Printf("---------start DMA %s----------\n", get_bp_str(g_BPP->BPNum));
        }
        bRet = TRUE;
        break;
    case ACT_TYPE_STANDBY:
        if (TRUE == Sim_SendStandby())
        {
            bRet = TRUE;
        }
        break;
   ...
   case ACT_TYPE_ABNORMAL_SHUTDOWN_NOTWAIT_L3EMPTY:
...
#endif
        break;
    case ACT_TYPE_FW_UPDATE:
        if(TRUE == Sim_FirmwareUpdate())
        {
            bRet = TRUE;
        }
        break;
    case ACT_TYPE_INJECT_ERROR:
        ...
        bRet = TRUE;
        break;
    case ACT_TYPE_INJECT_CRCERR:
        memcpy(&CRCErrParamSlot[g_InjCRCErrTail],&g_CurAct->m_ErrInjParam,sizeof(ERR_INJ_PARRM));
        g_InjCRCErrTail++;
        bRet = TRUE;
        break;
    case ACT_TYPE_LLF:
    ...
    case ACT_TYPE_WEAR_LEVELING_STATISTIC:
        ...
        break;
    case ACT_TYPE_CMD_TYPE:
        get_HSRWCmdType(g_CurAct->ProtocolType, (U8*)&g_HSRWCmdType);
        bRet = TRUE;
        break;
    ...
    case ACT_TYPE_SECURITY_ENABLE_CHECK:
        if (TRUE == Sim_SecurityStatusCheck(g_CurAct->ActType))
        {
            bRet = TRUE;
        }
        break;
    case ACT_TYPE_SET_ERASECNT:
    {
        ERASECNT_SETTING *pEraseCntSetting = &g_CurAct->m_FlashEraseCntParam;
        Mid_FlashSetBlockErase(pEraseCntSetting->uMaxEraseCnt, pEraseCntSetting->uMinEraseCnt, pEraseCntSetting->ulMaxPercent);
        bRet = TRUE;
        break;
    }
    case ACT_TYPE_SET_FLASH_ERR_RATE:
    {
        FLASH_ERR_RATE *pFlashErrRate = &g_CurAct->m_FlashErrRate;
        Mid_FlashSetErrRate(pFlashErrRate->UECCRate, pFlashErrRate->RECCRate, pFlashErrRate->ProgramErrRate, pFlashErrRate->EraseErrRate);
        if (0 != pFlashErrRate->UECCRate ||
            0 != pFlashErrRate->RECCRate ||
            0 != pFlashErrRate->ProgramErrRate ||
            0 != pFlashErrRate->EraseErrRate)
        {
            g_bInjectError = TRUE;
        }
        bRet = TRUE;
    ...
    case ACT_TYPE_NVME_WRITE_ZERO:
    {
        if (TRUE == Sim_NVMeWriteZero())
        {
            bRet = TRUE;
        }
        break;
...
void ResetSectionParam(void)
{
    ...
    g_BPP->BPNum = 0;
    g_HSRWCmdType = HSRW_CMD_TYPE_NCQ;
}
...
BOOL DoEvtInitSection()
{
    ...
    if (NULL != g_CurAct)
    {
        ...
        if (TRUE == bDone)
        {
            if(g_CurAct->ActType == ACT_TYPE_INJECT_ERROR ||g_CurAct->ActType == ACT_TYPE_INJECT_CRCERR || g_CurAct->ActType == ACT_TYPE_LLF)
            {
...
BOOL check_flash_cond()
{
    ...
    if( (0 != pFCmdCond->FCmdConDoCnt) &&(pFCmdCond->FCmdConDoCnt >= pFCmdCond->FCmdConHitCnt))
    {
        ...
        case MOD2_PPO:
            ...
            ulPPO += 2;
            if(ulPPO >= LOGIC_PG_PER_BLK)
            {
                ulPPO = 0;
            }
            break;
        case RAND_PPO:
            pFCmdCond->PhyPg = rand() % (LOGIC_PG_PER_BLK - 1);
            break;
        default:
            break;
        }
...
void inject_error_init()
{
    ...
    memset(&CRCErrParamSlot[0],0,ERR_INJ_SLOT_MAX*sizeof(ERR_INJ_PARRM));
}
void module_run_init()
{
    char InjErrLog[256] ={0};
    char *pStrLogFolder = SIM_GetLogFileFloder();

    sprintf(InjErrLog, "%s\\InjectErrRecord.txt", pStrLogFolder);
    g_CurSec = NULL;
    g_CurCond = NULL;
    g_CurAct = NULL;
    ...
    hFileInjErr = OpenInjErrFile(InjErrLog);
}
...
HANDLE OpenInjErrFile(char* fileName)
{
    ...
    char strFileName[256] = {0};
    char *pStrLogFolder = SIM_GetLogFileFloder();
    sprintf(strFileName, "%s\\%s", pStrLogFolder, fileName);
