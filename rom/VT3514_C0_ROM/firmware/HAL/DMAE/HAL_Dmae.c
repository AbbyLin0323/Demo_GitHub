...
Description: 
  ...
  select for this command entry.
    On VT3514_A0, select MCU in rGLB_64;
...
------------------------------------------------------------------------------*/
void DMAE_TEXT_ATTR HAL_DMAESelMCU(U8 ucDMAECmdID)
{
    U8 ucMCUID;
    U8 ucMCUSel;
...
Description: 
    Copy data from DRAM/OTFB/SRAM to DRAM/OTFB/SRAM.
...
20140912    Kristin    Coding style uniform
------------------------------------------------------------------------------*/
LOCAL BOOL DMAE_TEXT_ATTR HAL_DMAESetValueLenLimit(const U32 ulDesAddr,const U32 ulLenByte, U32 ulValue)
{
    U8 ucCurCmdID;
    volatile DMAE_CMDENTRY *pCurCmdEntry;
    U8 ucDesAreaType;
...
void DMAE_TEXT_ATTR HAL_DMAESetValue(const U32 ulDesAddr,const U32 ulLenByte, U32 ulValue)
{
    ...
    while (ulRemainLen > 0)
    {
        ...
        while (FALSE == HAL_DMAESetValueLenLimit(ulCurDesAddr, ulCurSetLen, ulValue))
        {
        }
