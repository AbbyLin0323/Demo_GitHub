...
{  
...
#ifdef SIM
    else if ((ulAddrIn >= DRAM_START_ADDRESS) && (ulAddrIn < OTFB_START_ADDRESS)) /* DRAM */
    {
    ...
    {
        /*temporary patch for FW use DMAE at the DCACHE enabled address*/
...
/*---------------------------------------------------------------------------
Name: HAL_DMAEParamCheck
Description:
Check if the address and length are valid
Input Param:
U32 ulSrcAddr: source address
U32 ulDesAddr: destination address
U32 ulLenByte: data length in unit of byte
Output Param:
none
Return Value:
BOOL: TRUE- parameters are valid
FALSE- parameters are invalid
Usage:
Called by HAL_DMAECopyOneBlock and HAL_DMAESetValue
History:
...
LOCAL BOOL DMAE_TEXT_ATTR HAL_DMAECopyOneBlockLenLimit(const U32 ulDesAddr, const U32 ulSrcAddr, const U32 ulBlockLenInByte, U8 ucSramMCUID)
{
    ...
    pCurCmdEntry->bsLength = DMAE_GET_CMD_LEN(ulBlockLenInByte);
...
{
    if (FALSE == HAL_DMAEParamCheck(ulSrcAddr, ulDesAddr, ulBlockLenInByte))
    {
        DBG_Printf("HAL_DMAECopyOneBlock parameter error: ulSrcAddr=0x%x, ulDesAddr=0x%x, ulBlockLenInByte=0x%x\n", ulSrcAddr, ulDesAddr, ulBlockLenInByte);
        DBG_Getch();
    }
    ...
    // only in dram path, fw needs to patch the DMAE 32Bytes Buffer issue in HW design by a dram-read-cmd. JasonGuo
    if ((ulCurDesAddr >= DRAM_START_ADDRESS) && (ulCurDesAddr < OTFB_START_ADDRESS))
    {
        ulCurCopyLen = *(volatile U32 *)DRAM_START_ADDRESS;
    }    
#else
...
void DMAE_TEXT_ATTR HAL_DMAESramCopyOneBlock(const U32 ulDesAddr, const U32 ulSrcAddr, const U32 ulBlockLenInByte, U8 ucSramMCUID)
{
    ...
    if (FALSE == HAL_DMAEParamCheck(ulSrcAddr, ulDesAddr, ulBlockLenInByte))
    {
        DBG_Printf("HAL_DMAESramCopyOneBlock parameter error: ulSrcAddr=0x%x, ulDesAddr=0x%x, ulBlockLenInByte=0x%x\n", ulSrcAddr, ulDesAddr, ulBlockLenInByte);
        DBG_Getch();
    }
...
void DMAE_TEXT_ATTR HAL_DMAESramSetValue(const U32 ulDesAddr,const U32 ulLenByte, U32 ulValue, U8 ucMCUID)
{
    ...
    if (FALSE == HAL_DMAEParamCheck(INVALID_8F, ulDesAddr, ulLenByte))
    {
        DBG_Printf("HAL_DMAESramSetValue parameter error: ulDesAddr=0x%x, ulLenByte=0x%x\n", ulDesAddr, ulLenByte);
        DBG_Getch();
    }
