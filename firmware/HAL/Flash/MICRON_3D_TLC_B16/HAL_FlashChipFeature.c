...
------------------------------------------------------------------------------*/
void MCU12_DRAM_TEXT HAL_FlashInitSLCMapping(void)//pending
{
    U32 ulLogicPage;
...
BOOL HAL_FlashIsTrigCacheOp(U8 ucCurrCmdType)//pending
{
    ...
    /*SLC mode*/
    if (/*(NF_PRCQ_READ_MULTIPLN == ucCurrCmdType)||(NF_PRCQ_PRG_MULTIPLN == ucCurrCmdType)||*/ (NF_PRCQ_READ == ucCurrCmdType)
...
BOOL MCU12_DRAM_TEXT HAL_FlashIsBlockBad(U8 ucPu, U8 ucLun, U8 ucPlane, U16 usBlk)
{
    ...
    tRdReq.bsRdBuffId  = ((DRAM_DATA_BUFF_MCU2_BASE - DRAM_START_ADDRESS) / PHYPG_SZ) + 10;
