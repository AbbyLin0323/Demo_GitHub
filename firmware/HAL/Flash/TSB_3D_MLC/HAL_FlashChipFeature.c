...
Usage:
    called in L2 FW
History:
    20160504    abby    create for TSB 3D MLC
------------------------------------------------------------------------------*/
...
//TSB 3D MLC , retry related pending
/*------------------------------------------------------------------------------
...
History:
    20160504    abby    create, pending for FW policy
------------------------------------------------------------------------------*/
...
BOOL MCU2_DRAM_TEXT HAL_FlashRetryPreConditon(FLASH_ADDR *pFlashAddr)
{
    ...
    pNFCQEntry->bsPrcqStartDw = HAL_NfcGetPrcqStartDw(NF_PRCQ_READRETRY_PRE);
    HAL_NfcCmdTrigger(pFlashAddr, NF_PRCQ_READRETRY_PRE, FALSE, INVALID_2F);
...
BOOL MCU2_DRAM_TEXT HAL_FlashRetrySendParam(FLASH_ADDR *pFlashAddr, RETRY_TABLE *pRetryPara, BOOL bTlcMode, U8 ucParaNum)                            
{
    ...
    {
        ucReqType = NF_PRCQ_READRETRY_ADJUSTMENT_CH0 + ucCH;
    }
    else//TLC retry
    {
        ucReqType = NF_PRCQ_TLC_READRETRY_ADJUSTMENT_CH0 + ucCH;
    }
...
BOOL MCU2_DRAM_TEXT HAL_FlashRetryEn(FLASH_ADDR *pFlashAddr, BOOL bRetry)
{
    ...
    pNFCQEntry->bsPrcqStartDw = HAL_NfcGetPrcqStartDw(NF_PRCQ_READRETRY_EN);
    HAL_NfcCmdTrigger(pFlashAddr, NF_PRCQ_READRETRY_EN, FALSE, INVALID_2F);
...
BOOL MCU2_DRAM_TEXT HAL_FlashRetryTerminate(U8 ucPU, U8 ucLun, BOOL bTlcMode)
{
    ...
    #ifdef SIM
    g_RetryTimes[HAL_NfcGetPhyPU(ucPU)] = 0;
    #endif
