...
Others      : This file's flash cmd is for TSB 3D MLC 2PLN chip
              mode 1 PLN read cmd.
Modify      :
...
GLOBAL MCU12_VAR_ATTR PRCQ_TABLE g_aPrcqTable[NF_PRCQ_CNT] =
{
    ...
    {NF_PRCQ_RESET_NOSTS,               NF_NORMAL_CMD,  TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
...
#ifndef BOOTLOADER
    /*  MLC FULL PLN cmd, TSB 3D MLC is 2 PLN flash  */
...
#endif
    /* pio cmd, unit in DW*/
    {NF_PRCQ_READRETRY_PRE,             NF_PIO_CMD,     TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
    {NF_PRCQ_READRETRY_ADJUSTMENT_CH0,  NF_PIO_CMD,     TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
    {NF_PRCQ_READRETRY_ADJUSTMENT_CH1,  NF_PIO_CMD,     TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
    {NF_PRCQ_READRETRY_ADJUSTMENT_CH2,  NF_PIO_CMD,     TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
    {NF_PRCQ_READRETRY_ADJUSTMENT_CH3,  NF_PIO_CMD,     TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
    {NF_PRCQ_READRETRY_EN,              NF_PIO_CMD,     TRIG_CMD_OTHER,   INVALID_BIT(9),    0},
...
/* normal cmd PRCQ QE */
GLOBAL MCU2_DRAM_TEXT U8 l_aQETable[] =
{
    // NF_PRCQ_RESET 
    // FF  | bsy | finish
    ...
    // NF_RCMD_INDEX_PRG_1PLN, LSB PAGE + MSB PAGE
    // 80 |  addr x 5 | data(pln0) |1A|
    // 80 |  addr x 5 | data(pln0) |10| bsy | finish
#ifdef FLASH_CACHE_OPERATION//pendig, need verify
    ...
    QE_GRP_ATTR(PRCQ_THREE_PHASE_CMD)   |QE_INDEX(CQE2E_1A81),
    QE_GRP_ATTR(PRCQ_FIVE_CYCLE_ADDR)   |QE_INDEX(ADDR1),
    QE_GRP_ATTR(PRCQ_DMA_WRITE)         |QE_INDEX(ADDR1),
    QE_GRP_ATTR(PRCQ_READ_STATUS_EH)    |QE_INDEX(SQEE_2E78),    
    QE_GRP_ATTR(PRCQ_ROW_ADDR)          |QE_INDEX(ADDR1),
    ...
    QE_GRP_ATTR(PRCQ_FINISH)            |QE_INDEX(END),
#else
    // 80|LSB PLN0 ADD X 5 | Data pln 0 |11 | BSY| 81 |LSB PLN1 ADD X 5 | Data pln 1 | 1A|
    // 80|MSB PLN0 ADD X 5 | Data pln 0 |11 | BSY| 81 |MSB PLN1 ADD X 5 | Data pln 1 | 10| BSY |END
    QE_GRP_ATTR(PRCQ_THREE_PHASE_CMD)   |QE_INDEX(CQE2E_1180),
    QE_GRP_ATTR(PRCQ_FIVE_CYCLE_ADDR)   |QE_INDEX(ADDR0),
    QE_GRP_ATTR(PRCQ_DMA_WRITE)         |QE_INDEX(ADDR0),
    QE_GRP_ATTR(PRCQ_READ_STATUS_EH)    |QE_INDEX(SQEE_2E78),    
...
{
    // NF_PRCQ_READRETRY_PRE, FOR MLC
    NF_RAW_CMD_READRETRYEN_L0,
    NF_RAW_CMD_READRETRYEN_L1,

    // NF_PRCQ_READRETRY_ADJUSTMENT_CH0
    RAW_PIO_CMD|PIO_CFG_VALUE(0x55),
    RAW_PIO_ADDR|PIO_CFG_VALUE(0x4),
    RAW_PIO_DATAOUT|PIO_CFG_VALUE(0x0),

    RAW_PIO_CMD|PIO_CFG_VALUE(0x55),
    RAW_PIO_ADDR|PIO_CFG_VALUE(0x5),
    RAW_PIO_DATAOUT|PIO_CFG_VALUE(0x0),

    RAW_PIO_CMD|PIO_CFG_VALUE(0x55),
    RAW_PIO_ADDR|PIO_CFG_VALUE(0x6),
    RAW_PIO_DATAOUT|PIO_CFG_VALUE(0x0),

    RAW_PIO_CMD|PIO_CFG_VALUE(0x55),
    RAW_PIO_ADDR|PIO_CFG_VALUE(0x7),
    RAW_PIO_DATAOUT|PIO_CFG_VALUE(0x0),

    RAW_PIO_CMD|PIO_CFG_VALUE(0x26)|RAW_PIO_IO_LAST_PHS(1),
    ...
    RAW_PIO_CMD|PIO_CFG_VALUE(0x5D)|RAW_PIO_IO_LAST_PHS(1),
