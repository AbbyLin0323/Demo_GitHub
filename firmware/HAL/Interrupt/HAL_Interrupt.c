...
/*------------------------------------------------------------------------------
Function:   HAL_L5TimerIntEntry
Input:  None
Output: None
Description : 
    Interrupt handler routine entry for Xtensa TIE Timer 2 (at Level 5)
Note:
Modify History:
    20160119    Yao Chen
------------------------------------------------------------------------------*/
...
void HAL_HostCIntInit(void)
{
    ...
    rHOSTC_INTMASK &=
        ~(INT_HOSTC_PERST_INBD | INT_HOSTC_PERST_DAST | INT_HOSTC_L12_IDLE |
        INT_HOSTC_NVME_SHUTDOWN | INT_HOSTC_NVME_CMDEN | INT_HOSTC_PCIEFAILURE |
        INT_HOSTC_AHCI_HRST | INT_HOSTC_MPTOOL | INT_HOSTC_FATALERR);
#endif
