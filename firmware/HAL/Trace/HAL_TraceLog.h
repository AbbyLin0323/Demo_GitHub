...
#define SUBSYSTEM_TL_MEM_SIZE_MAX     (128 * 1024 * 1024) //set max TL memory size to 128MB for SubSystem


/* main location definition area */
typedef struct _performance_tl_info
{
    U32 ulCurrentTime;
    U32 ulDiffCycles;
}PTL_INFO;
...
extern MCU12_VAR_ATTR volatile U32 g_ulPerformanceLevel;
extern MCU12_VAR_ATTR PTL_INFO g_tPtlInfo;
extern MCU12_VAR_ATTR PTL_INFO g_tPtlNullInfo;

#define    PTL_LEVEL_NULL       0
#define    PTL_LEVEL_TOP        BIT(0)
#define    PTL_LEVEL_SCHEDULE   BIT(1)
#define    PTL_LEVEL_DETAIL     BIT(2)
#define    PTL_LEVEL_ALL        0xff

#define TL_PERFORMANCE(_LEVEL_, ...) do \
{\
    if (g_ulPerformanceLevel & (_LEVEL_)) \
    {\
        g_tPtlInfo.ulDiffCycles = COM_DiffU32(g_tPtlInfo.ulCurrentTime, HAL_GetMCUCycleCount());\
        TL_TraceLog( TL_FILE_NUM, __LINE__, (void*)&g_tPtlInfo, sizeof(PTL_INFO));\
        g_tPtlInfo.ulCurrentTime = HAL_GetMCUCycleCount();\
    }\
    else\
    {\
        if (PTL_LEVEL_NULL == (_LEVEL_))\
        {\
            TL_TraceLog( TL_FILE_NUM, __LINE__, (void*)&g_tPtlNullInfo, sizeof(PTL_INFO));\
        }\
    }\
}while(0)
