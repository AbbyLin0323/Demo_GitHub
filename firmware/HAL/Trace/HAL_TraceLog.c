...
MCU12_VAR_ATTR volatile U32 g_ulPerformanceLevel = PTL_LEVEL_NULL;
MCU12_VAR_ATTR PTL_INFO g_tPtlInfo;
MCU12_VAR_ATTR PTL_INFO g_tPtlNullInfo = {0,0};
#endif
...
void TL_Initialize(U8 ucMcuId, void* pBuffer, U32 ulSize, U32 ulSecSize)
{
    ...
    l_pTlInfo = (TL_INFO *)(OTFB_TLG_MCU012_SHARE_BASE + (ucMcuId - MCU0_ID) * sizeof(TL_INFO));
...
History:
    ...
    20150407    Gavin   fix a bug in case of skipping a sector
------------------------------------------------------------------------------*/
...
{
    return (TL_INFO *)(OTFB_TLG_MCU012_SHARE_BASE + (ulMcuId - MCU0_ID) * sizeof(TL_INFO));
}
...
{
    TL_INFO* pTlInfo = (TL_INFO *)(OTFB_TLG_MCU012_SHARE_BASE + (ulMcuId - MCU0_ID) * sizeof(TL_INFO));
...
MCU12_VAR_ATTR U32 g_ulLastTime = 0;

void PTL_TestStart(void)
{
    g_ulLastTime = HAL_GetMCUCycleCount();
}

void PTL_TestEnd(void)
{
    static U32 ulCycleMax = 0;
    static U32 ulCycleMin = 0;
    static U32 ulCycleCount = 0;
    static U32 ulLoopTime = 0;
    U32 ulAverage;
    U32 ulDiffCycles;

    ulDiffCycles = COM_DiffU32(g_ulLastTime, HAL_GetMCUCycleCount());

    ulCycleCount += ulDiffCycles;
    ulLoopTime++;

    if (ulDiffCycles > ulCycleMax)
    {
        ulCycleMax = ulDiffCycles;
    }
    else if (ulDiffCycles < ulCycleMin)
    {
        ulCycleMin = ulDiffCycles;
    }
    
    if ((100000 == ulLoopTime) || (ulCycleCount > 0xfff00000))
    {
        ulAverage = ulCycleCount/ulLoopTime;
        DBG_Printf("%d loop average cycle:%d  max:%d  min:%d\n", 
        ulLoopTime, ulAverage, ulCycleMax, ulCycleMin);
        ulCycleCount = 0;
        ulLoopTime   = 0;
        ulCycleMax   = 0;
        ulCycleMin   = ulAverage;
    }
