...
// chain number manager
// please note that both MCU1(L1 and L2) and MCU2(L3) have to access the chain
// number manager data structure
MCU12_VAR_ATTR CHAIN_NUM_MGR* l_aChainNumMgr;

extern void HAL_SgeFinishChainCnt(U8 ucHID, U16 usTotalChain);
#ifdef SIM
...
{
    // add finish sector count of a host command,
    // please note that both MCU1 and MCU2 call this function

    CHAIN_NUM_MGR* pChainNumMgr = &l_aChainNumMgr[ucHID];
#ifdef SIM
    GET_CHAIN_NUM_LOCK;
#else
    HAL_MultiCoreGetSpinLockWait(SPINLOCKID_CHAIN_COUNT);
#endif
    ...
    if (pChainNumMgr->usTotalSecCnt == pChainNumMgr->usFinishSecCnt)
    {
        /*set total chain num*/
...
#else
    HAL_MultiCoreReleaseSpinLock(SPINLOCKID_CHAIN_COUNT);
#endif
