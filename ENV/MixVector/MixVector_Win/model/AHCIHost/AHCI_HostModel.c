...
void AHCI_HostReadDataFromBuffer(PLARGE_INTEGER BufferAddr,U32 DstLba)
{
...
//#endif
    ...
    if (0 == ulData)//host did not write this LBA, do NOT check
    {
        ...
        {
            DBG_Printf("AHCI_HostReadDataFromBuffer: High Lba 0x%x, DataCnt = 0, Lba in Buffer = 0x%x\n", 
        ...
        {
            DBG_Printf("AHCI_HostReadDataFromBuffer: Low Lba 0x%x, DataCnt = 0, data in Buffer = %d\n", 
    ...
    {
        DBG_Printf("AHCI_HostReadDataFromBuffer: High Lba in Buffer = 0x%x, Lba in CMD = 0x%x\n", 
                    BufferAddr->HighPart,DstLba);
    ...
    {
        DBG_Printf("AHCI_HostReadDataFromBuffer: Low Lba = 0x%x, data in Buffer = %d, data in Host = %d\n", 
...
void Host_ReadFromDeviceInterface(U32 HostAddrHigh, U32 HostAddrLow, U32 BufferAddr, U32 ByteLen, U8 nTag)
{
    ...
    if (TRUE == bLbaCmd)
    {
        ...
        {
            //do data check
            AHCI_HostReadDataFromBuffer((PLARGE_INTEGER)ullHostAddr.QuadPart, ulLbaIndex);
...
void AHCI_HostReadFromOTFB(U32 HostAddrHigh, U32 HostAddrLow, U32 OTFBAddr, U32 ByteLen, U8 nTag)
{
    ...
    {
        DBG_Printf("AHCI_HostWriteToOTFB: HostAddr is not align by 512 bytes \n");
        DBG_Break();
    }
#endif
...
void AHCI_HCmdToRowCmd(HCMD_INFO* pHcmd)
{
    ...
    {
    case CMD_TYPE_READ:
        pCmdFis->Command = ATA_CMD_READ_FPDMA_QUEUED;
        break;
    case CMD_TYPE_WRITE:
        pCmdFis->Command = ATA_CMD_WRITE_FPDMA_QUEUED;
        break;
    case CMD_TYPE_TRIM:
    ...
    if (ATA_CMD_READ_FPDMA_QUEUED == pCmdFis->Command ||
        ATA_CMD_WRITE_FPDMA_QUEUED == pCmdFis->Command) 
    {
        pCmdFis->Feature7_0 = (UCHAR)(SecCnt & 0x000000ff);
        pCmdFis->Feature15_8 = (UCHAR)((SecCnt & 0x0000ff00) >> 8);
    } 
    else if (ATA_CMD_VENDER_DEFINE == pCmdFis->Command)
    {
