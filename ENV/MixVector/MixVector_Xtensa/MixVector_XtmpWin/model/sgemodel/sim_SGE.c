...
{
#ifndef VT3514_C0
    l_ulDrqDepth[0] = DRQ_DEPTH_MCU1;
    l_ulDrqDepth[1] = DRQ_DEPTH_MCU2;
    l_ulDwqDepth[0] = DWQ_DEPTH_MCU1;
    l_ulDwqDepth[1] = DWQ_DEPTH_MCU2;
    l_aDrqStsRegAddr[0] = DRQ_MCU1_STS_REG_ADDR;
    l_aDrqStsRegAddr[1] = DRQ_MCU2_STS_REG_ADDR;
    l_aDwqStsRegAddr[0] = DWQ_MCU1_STS_REG_ADDR;
    l_aDwqStsRegAddr[1] = DWQ_MCU2_STS_REG_ADDR;
    l_aDrqBaseAddr[0] = DRQ_BASE_ADDR_MCU1;
    l_aDrqBaseAddr[1] = DRQ_BASE_ADDR_MCU2;
    l_aDwqBaseAddr[0] = DWQ_BASE_ADDR_MCU1;
    l_aDwqBaseAddr[1] = DWQ_BASE_ADDR_MCU2;
#else
...
void SGE_RecordChainNumForMCU1(U8 Tag ,U16 ChainNum, BOOL bNfcChain)
{
    ...
    if(TRUE == l_aChainNumRecord[Tag].Mcu1Valid
       && TRUE == l_aChainNumRecord[Tag].Mcu1DiscardChain)
    {
        DBG_Getch();
    }
    if (TRUE == bNfcChain)
    {
        l_aChainNumRecord[Tag].NfcChainNum1 = ChainNum;
    }
    else
    {
        l_aChainNumRecord[Tag].ChainNum1 = ChainNum + l_aChainNumRecord[Tag].NfcChainNum1;   
...
{
    EnterCriticalSection(&l_CriticalChainNumAdd);
    if(TRUE == l_aChainNumRecord[Tag].Mcu2Valid
       && TRUE == l_aChainNumRecord[Tag].Mcu2DiscardChain)
    {
        DBG_Getch();
    }
    if (TRUE == bNfcChain)
    {
        l_aChainNumRecord[Tag].NfcChainNum2 = ChainNum;
    }
    else
    {
        l_aChainNumRecord[Tag].ChainNum2 = ChainNum + l_aChainNumRecord[Tag].NfcChainNum2;   
...
{
    /* check if MCU1/2 is doing error handing or not */
    if(TRUE == l_aChainNumRecord[Tag].Mcu1DiscardChain              ///  error occurs  force to finish
        || TRUE == l_aChainNumRecord[Tag].Mcu2DiscardChain)
    {
        return TRUE;
    }
    else if(FALSE == l_aChainNumRecord[Tag].Mcu1Valid               //   One or both MCU not completed
...
LOCAL void ProcessDrqEntry(SGE_ENTRY *PDrqEntry)
{
    ...
    do
    {
        ...
        Host_ReadFromDram(
                      ...
                      ullHostAddr.LowPart,
                      tCurDsg.ulDramAddr + (tCurDsg.bsXferByteLen - ulCurDsgRemLen),
                      ulXferLen,
                      PDrqEntry->bsHID);
...
------------------------------------------------------------------------------*/
void SGE_OtfbToHost(U8 PU ,U8 Level, BOOL bNfcInjErr)
{
    HSG_ENTRY tHsgEntry;
    U16 usNextHsgId;
    SGE_ENTRY *pSgqEntry;
    ...
    ulOtfbAddr = SGE_GetOtfbAddr(PU);
    
    pSgqEntry = (SGE_ENTRY*)SGE_GetSgqEntry(PU,Level);
...
{
#ifdef SIM
    U32 ulSgeEntry = SGQ_BASE_ADDR + ((PU << NFCQ_DEPTH_BIT) + Level)*sizeof(SGE_ENTRY);
#else
    U32 ulSgeEntry = (U32)GetVirtualAddrInLocalMem(SGQ_BASE_ADDR + ((PU << NFCQ_DEPTH_BIT) + Level)*sizeof(SGE_ENTRY));//SGQ_BASE_ADDR + ((PU << NFCQ_DEPTH_BIT) + Level)*sizeof(SGE_ENTRY) - SRAM0_START_ADDRESS + (U32)g_pLocalRam0;
#endif
