...
#endif
#define DISK_B_C_MULTI
#define DISK_B_RD_ONLY
...
#define MIN_C_HSG_BYTE_LEN (512)
#define MIN_B_HSG_BYTE_LEN 8
#define HSG_CNT 64
...
//	    DBG_Printf("c check addr:0x%x data:0x%x\n",Addr,(Offset));
            for(len=0;len<Length;len++)
            {
                ...
                #if 0
                if(((Offset+len)%(SEC_PER_PG)==0)&&(len!=0))
                {
                    pu++;
                }
                #endif
...
VOID MixvLbaToFlash(FLASH_ADDR * p_flash_addr,U32 Lba,U32 LbaCnt)
{
    ...
    for(Page=StartPage;Page<(EndPage);Page++)
    {
        ...
        p_flash_addr[Index].Block = (Page>>(CE_BITS+PG_BITS))&(BLOCK_PER_CE-1);
        ...
        if(MskCmdComplete!=0)
       {
        ...
        if(Tag!=INVALID_8F)
        {
            ...
            if(Type==HOST_REQ_WRITE)
            {
                ...
                //DBG_Printf("wt pu:%d blk:%d pg:%d\n",HostCmdTable[Tag].FlashAddrGroup[i].PU,HostCmdTable[Tag].FlashAddrGroup[i].Block,HostCmdTable[Tag].FlashAddrGroup[i].Page);
...
{
    U32 hsgindex,totalhsglen,grandurity;
    USHORT *pStartHsg;
    pStartHsg = pHsg;
...
DISK_CMD_STATUS MixvDiskABSchdule(HOST_CMD_DPTR * pReqDptr,ULONG *offset,ULONG *length,HOST_REQ_TYPE * p_req)
{
    ...
    {
#ifndef DISK_B_RD_ONLY
        //disk a wt finish build rd req
        if(*rdFinishLen==diskByteLen)
#endif
        {
//	    DBG_Printf("rd finish\n");
            return RD_DISK_FINISH;
        }
        if(*rdOffset==diskByteLen)
        {
	    DBG_Printf("rd cmd send finish!\n");
            return RD_DISK_CMD_SEND_FINISH;
        }
        *offset = *rdOffset;
        *length = *rdLen;
...
//        g_cmd_dptr[DiskIndex].CurrDiskBaseCmdSize =
        cmdstatus = MixvDiskABSchdule(&g_cmd_dptr[DiskIndex],pOffset,pLength,Req);
...
//	DBG_Printf("Data Check Slot:%d Disk:%d HostAddr:0x%x ByteLen:0x%x\n",Tag,DiskIndex,Addr,Length);
        ...
        if(DiskIndex!=SUB_DISK_C)
        {
            for(i=0;i<(Length);i++)
            {
                ...
                if(DiskIndex==SUB_DISK_A)
                {
                    DataActual = ((Offset+i/4)<<DATA_RANDON_OFFSET_BIT);//(((Offset&0xffff)<<16)|(Offset&0xffff));//(t<<24)|(Offset);
                }
                if(DiskIndex==SUB_DISK_B)
                {
    ...
    for(DiskIndex=0;DiskIndex<SUB_DISK_CNT;DiskIndex++)
    {
        if((Cmd_Manager->HostCmdTable.SubDiskCmd[ DiskIndex ].DiskEn == TRUE)&&(Cmd_Manager->HostCmdTable.SubDiskCmd[ DiskIndex].ReqType == HOST_REQ_WRITE))
        {
            ...
            else
            {
                for(i=0;i<(Len>>2);i++)
                {
                    *(volatile ULONG *)Addr = ((Offset+i/4));//(((Offset&0xffff)<<16)|(Offset&0xffff));
                    Addr+=4;
                }
    ...
    for(CmdSlot=0;CmdSlot<MAX_CMD_SLOT;CmdSlot++)
    {
        switch(g_HostCmdManager[CmdSlot].State)
        {
            ...
            case GET_CMD_SLOT:
                ...
                break;
            case TRIG_CMD:
		tmpstartunit =g_HostCmdManager[CmdSlot].HostCmdTable.SubDiskCmd[SUB_DISK_B].StartUnit;

		tmpreq =g_HostCmdManager[CmdSlot].HostCmdTable.SubDiskCmd[SUB_DISK_C].ReqType;
		tmpunitlen =g_HostCmdManager[CmdSlot].HostCmdTable.SubDiskCmd[SUB_DISK_C].UnitLength;
//                DBG_Printf("trig slot:%d type:%d startunit:0x%x unitlen:0x%x\n",g_HostCmdManager[CmdSlot].TrigSlot,tmpreq,tmpstartunit,tmpunitlen);
...
VOID MixvAllInOnePattenInit()
{
    ...
    g_cmd_dptr[SUB_DISK_A].CurrDiskBaseCmdSize = 16;
    g_cmd_dptr[SUB_DISK_A].CurrDiskSize = 4*1024;
    ...
    g_cmd_dptr[SUB_DISK_C].CurrDiskBaseCmdSize = 32*1024;
