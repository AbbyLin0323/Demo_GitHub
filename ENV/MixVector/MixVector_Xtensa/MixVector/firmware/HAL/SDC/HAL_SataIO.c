...
void HAL_SataEnableNcqFinishInt(void)
{
	...
	rSDC_PHYControl12 |= BIT_FW_CMDINT_EN;
...
Return Value:
    none
Usage:
    called in boot/init stage.
History:
    20141016    Gavin   modify for coding style
------------------------------------------------------------------------------*/
LOCAL void SataPHYConfig(void)
{
    //TODO, sync with system team
...
------------------------------------------------------------------------------*/
void HAL_SDCFeatureInit(void)
{
    rTOP_REGGLB40 |= (1 << 26);//enable PRD in SRAM
    /* cache status in OTFB */
    rSACMDM_CACHE_STATUS_BASE_ADDRESS_SRAM = (U32)(OTFB_CACHE_STATUS_MCU1_BASE - OTFB_START_ADDRESS);

#ifdef FIS_Delay
    rSDC_FISDelayControl |= TXSETDEVCOMPL_DLY_EN | 0xC0;
#endif

    //rSDC_SHR_LockControl |= BIT_FW_CMDINT_EN;//interrupt when NCQ command finish
    
    /* enable HW to wait rSDC_SendSDBFISReady when DMA command */
    rSDC_ControlRegister = FW_DMAEXE_STE_EN;// | DIPM_EN;
    ...
    /*  PMU_Regitser_Base: 0x1ff81f00
        ...
        REG10[21] : 0 = FW_COMINIT_RDY is from PMU register FW_COMINIT_RDY_SUS= REG10[22];
    ...
    */
#ifdef VT3514_C0
    *(volatile U32 *)(REG_BASE_PMU + 0x10) |= 0x2d30000;
    *(volatile U32 *)(REG_BASE_PMU + 0x10) &= ~(1 << 21);//REG10 = (1<<25)|(1<<23)|(1<<22)|(0<<21)|(1<<20)|(1<<17)|(1<<16);
#else
    *(volatile U32 *)(REG_BASE_PMU + 0x10) |= 0x2f30000;
#endif
...
#ifdef ASR_Enable
    *(volatile U32*)(0x1ff81f10) |= 1 << 25 ;//PMU RST_COMINIT_EN 
    *(volatile U32*)(0x1ff80804) |= 1 << 24 ;//AsynRcovSup
    *(volatile U32*)(0x1ff80810) |= 1 << 9 ;//ELECIDLE dalay
#endif
...
#ifdef VT3514_C0
    /* In C0 HW, SDC can read COM_INIT ready flag from PMU side, FW do not need to handle COM_RESET */
    rSDC_IntMask |= BIT_SDC_INTSRC_COMRESET_RCV;
#endif
...
/*===================================================================
Function    : HAL_SataSendBadStatus
Input       : none
Output      : none
Description : send bad status in COMRESET and software reset protocol
Note: 
Modify History:
20090528    Yao Chen    001: first created
20150115    Kristin     modify code comments
...
void HAL_SataSendBadStatus(void)
{
    ...
    rSDC_SECCNT = 1;
    rSDC_DEVICE_HEAD = 0;
    rSDC_FISDirInt &= (~BIT_SDC_FIS_INTFLAG);//I=0
    HAL_SataSendRegD2HFIS();
    while ( FALSE == HAL_SataIsFISXferAvailable() );
...
/****************************************************************************
Name        :HAL_GetSATAEPHYState
Input       :None.
Output      :Current SATA EPHY power state, including both TX state and RX state.
Author      :Yao Chen
Date        :
Description : This routine reads SATA EPHY power state from hardware register.
Others      :
Modify      :
20141016    Gavin   ported from HAL_PM.c
****************************************************************************/
LOCAL INLINE U8 HAL_GetSATAEPHYState(void)
{
    /* Lower 4 bits in Reg 0xC9 */
    return 0;//( rSDC_PHYStatus & 0x0F );
}
