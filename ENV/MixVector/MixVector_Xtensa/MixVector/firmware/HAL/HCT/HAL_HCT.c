...
{
#ifndef VT3514_C0
    rGlbSoftRst |= R_RST_HCT; //set HCT reset bit to 1
#else
    rPMU_HOSTC_RST |= R_RST_HCT; //set HCT reset bit to 1
#endif

    HAL_DelayCycle(20);  //wait 20 clock

#ifndef VT3514_C0
    rGlbSoftRst &= ~R_RST_HCT; //clear HCT reset bit to 0
#else
    rPMU_HOSTC_RST &= ~(R_RST_HCT); //clear HCT reset bit to 0
#endif
...
#ifndef VT3514_C0
    if (FALSE == l_pHwFCQReg->bsFCQEMPT)
#else
...
Input Param:
    U32 ulMsgData: Message Data¨CVendor_Defined
