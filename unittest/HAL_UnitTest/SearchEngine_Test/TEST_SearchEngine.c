    ...
    for(ucSearchCase =0 ; ucSearchCase < SEARCH_CASE_MAX; ucSearchCase++)
    {
        ...
        switch(ucSearchCase)
        {
            case ERASE_CNT_MAX:
                ...
                l_aSECaseParam[ERASE_CNT_MAX].ulItemCnt = (BLK_PER_PLN + RSV_BLK_PER_PLN);
            ...
            case BACKUP_ERASE_CNT_MAX:
                ...
                l_aSECaseParam[BACKUP_ERASE_CNT_MAX].ulItemCnt = (BLK_PER_PLN + RSV_BLK_PER_PLN);
            ...
            case BACKUP_ERASE_CNT_MIN:
                ...
                l_aSECaseParam[BACKUP_ERASE_CNT_MIN].ulItemCnt = (BLK_PER_PLN + RSV_BLK_PER_PLN);
            ...
            case DIRTY_CNT_MAX:
                ...
                l_aSECaseParam[DIRTY_CNT_MAX].ulItemCnt = BLK_PER_PLN;
            ...
            case DIRTY_CNT_MIN:
                ...
                l_aSECaseParam[DIRTY_CNT_MIN].ulItemCnt = BLK_PER_PLN;
...
void TEST_SEVBTInit(BOOL bOTFB)
{
    ...
    for (i = 0; i < SUBSYSTEM_SUPERPU_MAX; i++)
    {
        for (j = 0; j<BLK_PER_LUN; j++)
        {
            k = i*BLK_PER_LUN + j;
            tVbtEntryTemp.BlockType = 0x3;
            tVbtEntryTemp.Rsvd = 0xAB;
            tVbtEntryTemp.PhysicalBlockAddr[0] = 0xFFFF;
...
void TEST_SEVbtMax(BOOL bOTFB)
{
    ...
    usStripeId = l_ulTimer%SUBSYSTEM_SUPERPU_MAX;//( 1 + g_ulLoopCnt ) % (1 << 16 );//can modify
    ...
    ulSWRtnValLow = *((U32*)&l_pVBT->m_VBT[ulSWRtnIndex / BLK_PER_LUN][ulSWRtnIndex%BLK_PER_LUN]);
    ulSWRtnValHigh = *((U32*)&l_pVBT->m_VBT[ulSWRtnIndex / BLK_PER_LUN][ulSWRtnIndex%BLK_PER_LUN] + 1);
    ...
    pSEParam->ulItemCnt = SUBSYSTEM_SUPERPU_MAX*BLK_PER_LUN;
    pSEParam->bsOTFB = bOTFB;

    HAL_SESearchValue(pSEParam, SE_1ST_MSK_GROUP, &tSEResult);
...
void TEST_SEVbtSame(BOOL bOTFB)
{
    ...
    SE_SEARCH_VALUE_RESULT tSEResult;
    U16 usStripeId;
    U32 ulDirtyCntToSearch = (((2 + 4 * (l_ulTimer%BLK_PER_LUN)) + 5 + l_ulTimer) * 3456) % (0x1 << 16);
    U16 usSWDirtyCnt;
    U32 ulSWRtnValLow;
    U32 ulSWRtnValHigh;
    U32 ulSWRtnIndex;
    ...
    usStripeId = l_ulTimer % SUBSYSTEM_SUPERPU_MAX;
...
search_end:
    ...
    tSEParam.ulItemCnt = SUBSYSTEM_SUPERPU_MAX*BLK_PER_LUN;
    ...
    HAL_SESearchValue(&tSEParam, SE_1ST_MSK_GROUP, &tSEResult);
